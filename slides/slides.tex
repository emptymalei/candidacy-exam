...
\begin{equation*}
    P_{1\to 2}^{(n)} = \frac{ \lvert \Gamma^{(n)}/2 \rvert^2 }{ \left\lvert  \Gamma^{(n)}/2 \right\rvert^2 + ( n \hat k - 1 )^2  } \sin^2 \left( \frac{ q^{(n)} }{2} x \right)  ,
\end{equation*}

where

\begin{align*}
    q^{(n)} &= \sqrt{\left\lvert  \Gamma^{(n)}/2 \right\rvert^2 + ( n \hat k - 1 )^2},\quad \text{frequency of oscillations} \\
