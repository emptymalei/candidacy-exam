...
\begin{figure}
    ...
    \caption*{Probabilities of finding neutrinos to be in each flavors.}
...
\end{equation*}
\alert{Mixing angle determines flavor oscillation amplitude\\
$\omega_m=(m_2^2-m_1^2)/2E$ determines oscillation wavelength}
...
\begin{align*}
    {\color{red}\omega_{\mathrm{m}}(x) } = & ~\omega_{\mathrm{v}}\sqrt{ \left(\frac{\lambda(x)}{\omega_{\mathrm{v}}} - \cos 2\theta_{\mathrm{v}} \right)^2 + \sin^2 2\theta_{\mathrm{v}}  }  ,\\
    {\color{blue}\tan 2\theta_{\mathrm{m}}(x)} = & ~\frac{\sin 2\theta_{\mathrm{v}}}{\cos 2\theta_{\mathrm{v}} - \lambda(x)/\omega_{\mathrm{v}} }.
\end{align*}
\end{tcolorbox}
...
\begin{equation*}
    -\frac{\omega_{\mathrm{m}}}{2} \boldsymbol{\sigma_3}.
\end{equation*}
...
\begin{tcolorbox}
P. Krastev (1989); F. Loreti (1994); A. Balantekin, et al (1996); E. Akhmedov (2000); \\
J. Kneller, et al (2013); K. Patton, et al (2014); $\cdots$
\end{tcolorbox}
...
\caption*{Stimulated oscillations. $\delta \lambda(x) = A \sin (\hat k \hat x)$  with $\hat A=0.1$, $\hat k=0.995$, $\theta_{\mathrm{m}}=\pi/6$}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[height=0.6\textheight]{assets/supernova-shock-turbulence.png}
...
\begin{equation*}
    \begin{pmatrix}
    -\omega/2 & \alpha \omega e^{i k x}\\
    \alpha \omega  e^{-i k x} & \omega/2
    \end{pmatrix},
\end{equation*}
\end{tcolorbox}
...
where $J_n(\beta)$ is Bessel's function of the first kind.
...
\begin{align*}
    q^2 &= \left\lvert  \Gamma/2 \right\rvert^2 + ( n_0 \hat k - 1 )^2,\quad \text{frequency of oscillations} \\
    \Gamma &= \left\lvert \hat B_{n_0} \right\rvert, \quad \text{width of resonance ($n\hat k$ as parameter)} \\
...
\begin{figure}
    ...
    \caption*{Comparison of approximation and numerical solution for two-frequency matter profile}
\end{figure}
...
\begin{figure}
    ...
    \caption*{Supernova matter density power spectra from three simulations. E. Borriello, et al (2014)}
\end{figure}
...
\item
Simple periodic matter profile $\delta\lambda(x)\sim \sin(k x)$ $\to$ resonance and width
\end{itemize}
...
\begin{itemize}\color{red}
 ...
 \item
 Combine matter effect with neutrino self-interactions
\end{itemize}
...
I am very thankful to Dr. Sajad Abbar, Prof. Huaiyu Duan (my advisor), and Dr. Shashank Shalgar, for all the help in both research and life.

Supported by DOE EPSCoR grant \#desc0008142 at UNM.
