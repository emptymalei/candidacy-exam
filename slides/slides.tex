...
\pause
{
\includegraphics[width=0.7\linewidth,height=0.9\textheight,keepaspectratio]{assets/oscillations-three-flavor.eps}
}
...
\begin{tcolorbox}[title=What if flavor eigenstates are the energy eigenstates?]
The initial condition $\ket{\Psi(t=0)}=\ket{\nu_e}$ with eigen energy $E_1$. Then the state at any time is

\begin{equation*}
\ket{\Psi(t)} = \ket{\nu_e} e^{-iE_1 t}.
\end{equation*}

Survival probability for electron flavor neutrino is
...
\begin{description}
\item[Energy Eigenstates] Diagonalized Hamiltonian
\item[Flavor Eigenstates] Eigenstates of weak interaction
\end{description}
\end{tcolorbox}
...
\begin{equation*}
P(\ket{\nu_e}\to\ket{\nu_x})  = \frac{1}{2} \sin^2(2\theta) (1- \cos ( \Delta E t ) )
\end{equation*}
\end{tcolorbox}
