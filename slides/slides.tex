...
\begin{tcolorbox}
A. Balantekin, T. Ohlsson, A. Smirnov, E. Akhmedov, J. Kneller, et al
\end{tcolorbox}
...
\begin{equation*}
\hat h= \sum_{n=-\infty}^{\infty} \frac{1}{2} \hat B_n e^{i  {\color{red} (n\hat k-1) } \hat x},
\end{equation*}

where $\hat B_n =  - (-i)^n  n \hat k \tan 2\theta_{\mathrm{m}}  J_n ( \hat A \cos 2\theta_{\mathrm{m}} / \hat k ) $.


${\color{red} (n \hat k-1) }$ $\to$ perturbation frequency in matter profile for each component.
...
\centering
small ${\color{red} (n\hat k-1)}$ $\to$ important term

\centering
Find integer $n_0 = \mathrm{Round}\left[ 1/\hat k \right]$ that minimizes ${\color{red} n\hat k -1}$.
...
\begin{align*}
    ...
    \Gamma &= \left\lvert \hat B_{n_0} \right\rvert, \qquad \text{width of resonance ($n\hat k$ as parameter)} \\
